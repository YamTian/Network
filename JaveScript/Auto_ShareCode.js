...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      $persistentStore.write($.farmInfo.farmUserPro.shareCode,'JDFruitCode')
      $.JDFruittUrl = 'http://api.turinglabs.net/api/v1/jd/farm/create/' + $persistentStore.read('JDFruitCode') + '/'
    } else {
      /*console.log(
      ...
      try {
        if (err) {
          ...
          if (
            ...
            $persistentStore.write($.petInfo.shareCode,'JDPetCode')
            $.JDPetUrl = 'http://api.turinglabs.net/api/v1/jd/pet/create/' + $persistentStore.read('JDPetCode') + '/'
...
async function getJDBean() {
  ...
  async function jdPlantBean() {
    ...
    if ($.plantBeanIndexResult.code === "0") {
      ...
      $persistentStore.write($.myPlantUuid,'JDBeanCode')
      $.JDBeanUrl = 'http://api.turinglabs.net/api/v1/jd/bean/create/' + $persistentStore.read('JDBeanCode') + '/'
    } else {
      console.log(
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.data.shareTaskRes) {
                ...
                $persistentStore.write(data.data.shareTaskRes.itemId,'JDZZCode')
                $.JDZZUrl = 'https://code.chiang.fun/api/v1/jd/jdzz/create/' + $persistentStore.read('JDZZCode') + '/'
                } else {
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success && data.data && data.data.userInviteCode) {
              ...
              $persistentStore.write(data.data.userInviteCode,'CarzyJoyCode')
              $.CrazyJoyUrl = 'https://code.chiang.fun/api/v1/jd/jdcrazyjoy/create/' + $persistentStore.read('CarzyJoyCode') +'/'
            }
          }
        }
      } catch (e) {
...
// 上传疯狂的JOY互助码
function uploadCrazyJoy() {
  return new Promise((resolve) => {
    const url = { url: $.CrazyJoyUrl }
    $.get(url, (_err, resp, data) => {
      try {
         const obj = JSON.parse(data)
        if (obj.code == 200) {
...
function showMsg() {
  return new Promise((resolve) => {
    $.msg($.nickName || $.UserName + $.name, "", $.result.join('\n'));
    resolve();
  });
}
