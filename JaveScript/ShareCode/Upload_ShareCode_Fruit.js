const $ = new Env('上传东东农场互助码')
const FruitCode1 = $persistentStore.read('FruitCode1') || '';
const FruitCode2 = $persistentStore.read('FruitCode2') || '';

// 东东农场
$.JDFruittUrl1 = 'http://api.turinglabs.net/api/v1/jd/farm/create/' + FruitCode1 + '/'
$.JDFruittUrl2 = 'http://api.turinglabs.net/api/v1/jd/farm/create/' + FruitCode2 + '/'

$.result = []

// 分析
!(async () => {
    await uploadJDFruit1()
    await uploadJDFruit2()
    await showMsg()
})()
  .catch((e) => $.logErr(e))
  .finally(() => $.done())
...
// 东东农场
function uploadJDFruit1() {
  return new Promise((resolve) => {
    const url = { url: $.JDFruittUrl1 }
    $.get(url, (_err, resp, data) => {
      try {
        const obj = JSON.parse(data)
        if (obj.code == 200) {
          $.result.push("账号一 东东农场：添加成功")
        }else
		if(obj.code == 400) {
          $.result.push("账号一 东东农场：已存在")
        }else{
          $.result.push("账号一 东东农场：添加异常")
        }
      } catch (e) {
        $.logErr(e, resp)
      } finally {
...
// 东东农场
function uploadJDFruit2() {
  return new Promise((resolve) => {
    const url = { url: $.JDFruittUrl2 }
    $.get(url, (_err, resp, data) => {
      try {
        const obj = JSON.parse(data)
        if (obj.code == 200) {
          $.result.push("账号二 东东农场：添加成功")
        }else
		if(obj.code == 400) {
          $.result.push("账号二 东东农场：已存在")
        }else{
          $.result.push("账号二 东东农场：添加异常")
        }
      } catch (e) {
        $.logErr(e, resp)
      } finally {
