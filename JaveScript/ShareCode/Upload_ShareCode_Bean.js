const $ = new Env('上传种豆得豆互助码')
const BeanCode1 = $persistentStore.read('BeanCode1') || '';
const BeanCode2 = $persistentStore.read('BeanCode2') || '';

// 种豆得豆
$.JDBeanUrl1 = 'http://api.turinglabs.net/api/v1/jd/bean/create/' + BeanCode1 + '/'
$.JDBeanUrl1 = 'http://api.turinglabs.net/api/v1/jd/bean/create/' + BeanCode2 + '/'
...
// 种豆得豆
function uploadJDBean1() {
  return new Promise((resolve) => {
    const url = { url: $.JDBeanUrl1 }
    $.get(url, (_err, resp, data) => {
      try {
        const obj = JSON.parse(data)
        if (obj.code == 200) {
          $.result.push("账号一 种豆得豆：添加成功")
        }else
		if(obj.code == 400) {
          $.result.push("账号一 种豆得豆：已存在")
        }else{
          $.result.push("账号一 种豆得豆：添加异常")
        }
      } catch (e) {
        $.logErr(e, resp)
      } finally {
...
// 种豆得豆
function uploadJDBean2() {
  return new Promise((resolve) => {
    const url = { url: $.JDBeanUrl2 }
    $.get(url, (_err, resp, data) => {
      try {
        const obj = JSON.parse(data)
        if (obj.code == 200) {
          $.result.push("账号二 种豆得豆：添加成功")
        }else
		if(obj.code == 400) {
          $.result.push("账号二 种豆得豆：已存在")
        }else{
          $.result.push("账号二 种豆得豆：添加异常")
        }
      } catch (e) {
        $.logErr(e, resp)
      } finally {
