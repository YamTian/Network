const $ = new Env('上传东东萌宠互助码')
const PetCode1 = $persistentStore.read('PetCode1') || '';
const PetCode2 = $persistentStore.read('PetCode2') || '';

// 东东萌宠
$.JDPetUrl1 = 'http://api.turinglabs.net/api/v1/jd/pet/create/' + PetCode1 + '/'
$.JDPetUrl2 = 'http://api.turinglabs.net/api/v1/jd/pet/create/' + PetCode2 + '/'
...
// 东东萌宠
function uploadJDPet1() {
  return new Promise((resolve) => {
    const url = { url: $.JDPetUrl1 }
    $.get(url, (_err, resp, data) => {
      try {
        const obj = JSON.parse(data)
        if (obj.code == 200) {
          $.result.push("账号一 东东萌宠：添加成功")
        }else
		if(obj.code == 400) {
          $.result.push("账号一 东东萌宠：已存在")
        }else{
          $.result.push("账号一 东东萌宠：添加异常")
        }
      } catch (e) {
        $.logErr(e, resp)
      } finally {
...
// 东东萌宠
function uploadJDPet2() {
  return new Promise((resolve) => {
    const url = { url: $.JDPetUrl2 }
    $.get(url, (_err, resp, data) => {
      try {
        const obj = JSON.parse(data)
        if (obj.code == 200) {
          $.result.push("账号二 东东萌宠：添加成功")
        }else
		if(obj.code == 400) {
          $.result.push("账号二 东东萌宠：已存在")
        }else{
          $.result.push("账号二 东东萌宠：添加异常")
        }
      } catch (e) {
        $.logErr(e, resp)
      } finally {
